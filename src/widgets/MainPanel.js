...
class MainPanel extends BaseWidget {
    constructor(opts, screen, config) {
        super({top: '0', height: '99%', handleKeys: true, screen, ...opts});

        this.config = config;
        this.screen = screen;
        this.currentPage = opts.currentPage || 1;
        this.initialRow = opts.initialRow || 0;
        this.colSpacing = opts.colSpacing || 1;
    ...
    openLevelFilter() {
        const levels = ['all', 'debug', 'verb', 'info', 'warn', 'error'];
    ...
    openGoToLine() {
        this.setMode('GOTO');
        this.prompt('Line:', '', (value) => this.moveToLine(parseInt(value) - 1));
    }
    ...
    moveToCenterViewportLine() {
        this.row = parseInt((this.initialRow + this.pageHeight) / 2);
        this.renderLines();
    }
    ...
    update(notify = true) {
        ...
        const highlight = (row, index) => {
            ...
            if (index === this.relativeRow) {
                return `{#1e1e1e-bg}{white-fg}{bold}${str}{/}`;
            }
            return str;
        };
